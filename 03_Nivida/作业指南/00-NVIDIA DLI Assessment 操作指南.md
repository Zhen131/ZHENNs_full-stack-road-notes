...
状态：执行前准备完成，尚未运行最终 Assessment

目标：只操作 `06_Assessment.ipynb`，完成代码、训练模型、通过自动评分并领取证书。
...
- `06` 有 **22 个 `FIXME`**，另有一个需要根据生成效果调整的 `w`。
...
7. 最终提交前先检查模型输出；取得 Ethan 明确确认后，才运行 `run_assessment(...)`。
8. Notebook 明确显示通过后，回课程页点击 `ASSESS TASK`。
9. 课程页显示通过且证书可领取后，下载证书。
10. 离开前确认保存完成，再点击 `STOP TASK`。
...
`w` 没有保证一次成功的固定值。先运行 `w = 0.0` 观察基线；如果数字与目标类别不稳定，再依次尝试：

```text
0.5 → 1.0 → 2.0
```

每次只修改 `w`，重新运行设置 `w`、生成图片和形状检查相关的格；不需要重新训练模型。
...
| 输出形状 | `[10, 1, 28, 28]` |
| 图片 | 0–9 大多数肉眼可识别 |
| 最终授权 | Ethan 已明确确认可以运行 Assessment |
