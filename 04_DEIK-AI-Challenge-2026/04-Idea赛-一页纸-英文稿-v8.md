# 04 — Idea 赛 · 英文稿 v8（PDF 正文 + Comments 分区）

日期：2026-09-02　｜　状态：v8，PDF 正文已收口，待实测页数
排版：12 号字、单倍行距、Times New Roman、页边距 2cm、段间不空行（首行缩进分段）

> **v8 相对 v7：清掉五处行话，全部改成人话。**
> 触发点是产品负责人指出 `A buy landing in the 97th percentile of an asset's 24-hour move` 完全看不懂。复查后发现同类问题共五处：
> | 处 | 原文 | 问题 | 改后 |
> | --- | --- | --- | --- |
> | 第一层 | `the 97th percentile of an asset's 24-hour move` | 统计行话 | `at the top of a day that went up fourteen percent`——**与视频定稿用词完全一致** |
> | AI 段引语 | `you bought in the 97th percentile` | 同上，且与上文第二层的说法不统一 | `you bought at the top of a fourteen-percent day`，与第二层逐字一致，形成回声 |
> | AI 段开头 | `percentile statistics are arithmetic` | 同上 | `comparing dates and prices is arithmetic` |
> | 第三层 | `Absence correlates with impulse` | `correlates` 是统计词 | `The missing note and the impulsive trade turn up together` |
> | AI 段 | `Self-deception is not finite` / `enumeration always misses` | 抽象名词堆叠 | `There is no fixed list of ways to fool yourself` / `any list of them will always miss some` |
>
> **原则：视频里已经定过稿的说法，PDF 直接沿用。** 评委看完视频再读纸，会有「对上了」的感觉；而两处说法不一致反而会让人以为是两件事。
>
...
**The idea has three layers.** *One: read the behaviour, and ask nothing.* The ledger already knows the second each trade happened, and what the price was doing at that moment. If I bought at the top of a day that went up fourteen percent, that is already in the record. Nobody had to ask me how I felt. Trades do not lie.
...
*Three: silence is data.* People do not skip the note at random. They skip it when they are impulsive, when they feel caught out, when they are in a hurry. The missing note and the impulsive trade turn up together, so a gap in the writing is a signal, not a hole in the data. Layers one and three ask nothing of the user, so the system still works for someone who never writes a word.
...
**It must also be AI.** Two of the three layers do not need it — comparing dates and prices is arithmetic, and I will not call that AI. Knowing where not to use AI is how I know where to use it. The second layer is different, and it is not simply reading a sentence.

*The contradiction is in the meaning, not in the words.* "The fundamentals changed, I am here for the long term" and "you bought at the top of a fourteen-percent day" do not clash on the surface. They clash because someone who is really in it for the long term does not care where the price sits today — and the same excuse can be written a thousand different ways, so any list of them will always miss some, and it will miss the most original people first.
...
*A fixed rule only catches the excuses I already thought of.* There is no fixed list of ways to fool yourself, and the list grows as you learn about yourself — every time I am caught, I invent a better reason. No rule written in advance can keep up with that. The value of a model is not that it recognises the patterns I listed. It is that it finds the ones I did not.
