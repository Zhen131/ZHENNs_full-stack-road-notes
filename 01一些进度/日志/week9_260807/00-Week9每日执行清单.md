...
> 收口状态（2026-07-24）：Week 9 按调整后的 Gate 提前完成，源码 `main@4fadfb6` 已推送。本文保留原始日计划；其中“旧明文自动迁移”“手动锁定”“硬离线通过”已被最终边界替换：旧/未知格式拒绝且仅允许明确重置、不提供手动锁定按钮、硬离线已取消且未验证但不阻塞。完成状态以 [[00-Week9-Checklist]] 与 [[01B_W9-IndexedDB静态加密执行验收记录]] 为准。
...
- [x] KDF 测试通过
- [x] AES-GCM round-trip / tamper 测试通过
- [x] 旧明文 / 未知格式拒绝且不自动覆盖；失败写入安全保留旧记录
- [x] 首次设密、再次解锁与刷新后重新解锁通过
- [x] 导入后重新加密通过
- [x] 硬离线 Gate 已取消（未验证，不作为本轮阻塞项）
- [x] test / lint / build / diff-check 通过

结论：Week 9 按用户调整后的 Gate 为 Go；可以进入 Week 10 图表，不把已取消的硬离线项目写成通过。
