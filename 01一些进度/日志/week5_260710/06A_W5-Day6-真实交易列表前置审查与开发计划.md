...
文档状态：已纳入软件工程、架构安全与维护性审查意见，待用户最终复核

执行授权：本文件本身不构成源码修改、提交、合并或推送授权；未收到用户明确执行指令前，任何 AI 不得开始 Day 6 源码工作
...
1. 交易表格明确接收并渲染 `ledgerData.trades`。
...
执行 AI 必须先确认用户已经明确要求“执行 Day 6”或同等指令。本计划被读取、审查或提交，不等于允许修改源码。
...
3. 在 Dashboard 接线前先运行隔离测试；不强制为了形式制造红灯，但必须确认删除任一字段映射、类型映射或空状态都会导致对应测试失败。
4. 不为制造红灯而破坏现有代码。
...
只有代码提交和全部验证都成立，才允许更新 README。建议使用独立中文提交：
...
只有源码结果已经被验证，才允许更新根仓库状态与 Week 5 文档。
...
5. **测试证据**：定向测试、全量测试、lint、build、diff-check 的实际结果和数量。
6. **浏览器证据**：空状态、旧占位数据消失、控制台与 hydration 状态。
7. **Git 证据**：分别报告源码仓库和文档仓库的分支、提交、工作树、ahead/behind。
8. **未完成能力**：明确 Gate 4-5、排序、格式化、分页和持久化仍未完成。
9. **下一步**：只有用户要求时才建议合并、推送或进入 Week 6。
