...
const result = {
  identity: {
    ...
    verdictScope:
      "只判定 T1-03、T1-05 与 T5；夹具和图表仅用于建立场景，不得覆盖已按重跑规则记为 BLOCKED 的 T1-02 和 T1-04 pointerleave",
  },
  preflight: {},
  passwordSetup: {},
  fixture: {},
...
try {
  ...
  if (heatmapClickWorked) {
    ...
    result.heatmap.cancelledBySecondClick = await page
      ...
      .isVisible();
    if (!result.heatmap.cancelledBySecondClick) {
      await chartSection
        .getByRole("button", { name: "清除日期筛选", exact: true })
        .click();
    }
  }
  ...
  await writeFile(
    path.join(evidenceDir, "15-production-t1-t5-focused.json"),
