...
async function addTrade(page, trade) {
  ...
  await target.getByLabel("类型", { exact: true }).selectOption(trade.type);
  await target.getByLabel("资产", { exact: true }).selectOption(trade.asset);
...
async function addManualPrice(page, asset, priceValue, date) {
  ...
  await target
    .getByLabel("价格资产", { exact: true })
    .selectOption(asset);
  await target.getByLabel("当前价格", { exact: true }).fill(priceValue);
  await target.getByLabel("价格日期", { exact: true }).fill(date);
...
async function checkPasswordEye(page, label) {
  ...
  ensure(pointerLeaveType === "password", `${label}: pointerleave did not hide`);
...
try {
  ...
  await writeFile(
    path.join(evidenceDir, "11-production-main-chain.json"),
