...
自动门全绿后再执行。使用 production build，并记录实际启动命令和实际专用测试原点。hostname 可以是 `localhost` 或 `127.0.0.1`，端口可以是执行开始时选定的任意可用端口；一旦选定，CH-01～CH-14 必须始终使用同一个精确原点，协议、hostname 和端口均不得中途变化，也不得拼接不同原点的证据。

必须使用全新的隔离 Chrome Profile，或能够证明不存在未知用户连接、IndexedDB、文件句柄和权限状态的专用环境。不得为取得测试结果读取、忘记、清空、覆盖或重新绑定归属未知的旧账本连接。必须通过 macOS 系统原生文件选择器创建／选择文件；jsdom、组件测试、开发服务器随手查看、截图或应用内嵌浏览器都不能替代。
...
启用 Chrome DevTools 的 Console 与 Network “Preserve log”，开始前清空本次隔离环境的记录。所选专用测试原点的页面资源属于预期；Binance 请求只能紧跟明确点击，其他时刻不得出现。所有密码、资产、金额和备注均为一次性虚构值。
...
整条证据链记录实际启动命令、精确原点、Chrome 版本、源码 branch/HEAD、隔离 Profile 或等价环境的无未知状态依据和专用虚构 B/C 文件名；每一步记录预期与实际、Console error/warning 数和 Binance 请求触发点。记录中不得粘贴密码、密钥、完整 ciphertext 或任何个人账本。CH-01～14 任一失败、执行中更换原点、无法证明环境隔离，或触碰归属未知的旧连接，开发结果均为 `FAIL`；不能用自动测试、截图或另一次不同路径的成功抵消，重新取证必须从 CH-01 开始完整执行。
...
- production build、实际启动命令、Chrome 版本和实际专用测试原点均有记录；hostname 可为 `localhost` 或 `127.0.0.1`，端口可动态选择；
- CH-01～14 在真实 Google Chrome、同一个精确隔离原点、原生文件选择器和专用虚构 V3 文件中逐项通过，整条链没有中途更换协议、hostname 或端口；
- 使用全新隔离 Chrome Profile 或等价的无未知状态专用环境，且没有读取、忘记、清空、覆盖或重新绑定归属未知的旧账本连接；
...
独立执行者应使用全新的隔离 Chrome Profile，自行选择并记录一个可用的专用原点；hostname 可以是 `localhost` 或 `127.0.0.1`，端口可以是任意可用端口，但独立 CH-01～CH-14 全链必须保持同一协议、hostname 和端口。不得读取、清除、覆盖或重新绑定未知旧连接。
