...
## 更正（2026-09-01，依 `06D` 独立验收，由产品负责人执行）

`06D` 判本批为**有条件通过**，并指出本报告两处表述与最终代码不符。**两处均为报告表述问题，不是产品缺陷。** 原文一律保留不改写，更正如下。

### 更正一：R-17 中 S2-F 的通电检查结论，在最终提交态 `fd32c2e` 上不成立

第 606 行表格记载「临时关闭 `verifyLedgerFileForOpen` 中『current 失败而 previous 验证成功则返回 recovery-required』分支 → 1 failed」。

`06D` 按该记述在 `fd32c2e` 上原样复现，**测试没有变红，照常通过**。经追查，原因是最终实现存在**两条互相独立的恢复路径**（`src/platform/files/ledgerFileRepository.ts:418-428`）：

```text
olderHeaderRecovery = index > 0
verified.status === "recovery-required" || olderHeaderRecovery
```

即：除「current 认证失败而 previous 成功」之外，「新头槽验证失败回落到旧头槽」同样产出 `recovery-required`。只关掉其中一条，另一条仍会兜住，故测试不变红；`06D` 把两条一起关掉后测试才变红。

**这说明该测试不是空的，产品的恢复能力真实存在，而且比合同要求更厚（两条独立路径而非一条）。**

**但本报告写下的那条通电结论，在最终代码上不成立。** 该结论取得于 S-2 阶段的 `2723b46`，当时 S-3 尚未引入第二条路径，结论**当时可能属实**；`06D` 未回到该提交复跑，产品负责人亦未复跑。**因此「S2-F 的通电检查在 S-2 阶段是否属实」记为「未核验」**，不主张它当时成立，也不主张它当时不成立。

**在 `fd32c2e` 上有效的通电检查方式**是：同时关闭上述两条恢复路径，测试变红；恢复任一条即变绿。该方式由 `06D` 实测确认。

**教训**：分阶段实施的批次中，**早期阶段取得的通电检查结论，不会自动在最终提交态继续成立**——后续阶段可能增加新的兜底路径，使原先的单点破坏不再充分。今后凡跨阶段的硬测试，**通电检查须在最终提交态重做一次**，或明确标注其取得于哪个提交、未在最终态复验。

### 更正二：V3 尾随空白构造的描述不准确

本报告把 V3 的尾随空白构造描述为「只给 JSON 头增加尾随空白并同步头长度，原始密文体不变」。

实际实现是**把整个 256 KiB 头槽清零后重写**，其中包含末尾 16 字节的 manifest 认证标签。

`06D` 确认该差异**不影响任何被保护的性质**：该测试守的仍是「语义有效但字节不完全一致的 candidate readback 不得被当成精确写入」，且原始密文体确实未变。仅描述不准确，据实更正。

### 未解除的条件

`06D` 的第二项条件——**性能数字未经第二方复现**——不属于报告表述问题，须由产品负责人明确裁决接受该证据等级。裁决结果另行记录，见 `99_W15` 当日日志。
