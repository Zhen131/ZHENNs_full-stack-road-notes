...
## 更正（2026-09-01，依 `07D` 第九节）

**本节由产品负责人一侧代为撰写，执行者未参与。上文原文一字不改写**，四条更正逐条另行列出。每条均由本方重新打开源码原文确认，不只是转述 `07D`。分支与提交未移动，仍为 `zhennn/w15-main-i18n-mechanism@0a3f5e0`；**本节不改动任何代码。**

### 更正一：R-1「均为纯 ASCII」不成立，`platform` 一处排序实际不等价

原文（R-1）称 15 处比较对象「均为 ISO 日期、资产代号、路径、relation 或技术 id 等纯 ASCII」。

**该表述有一处不成立。** `TradeForm.tsx:351` 的排序对象是 `ledgerData.feeRules[].platform`，即用户在手续费规则里**自由输入的平台名，无任何字符集约束**：

```ts
).sort((left, right) => (left < right ? -1 : left > right ? 1 : 0));
```

平台名大小写混用或含变音符号时，`<`／`>`（按 UTF-16 码元）与原 `localeCompare`（按语言习惯）**排序结果确实不同**。

**更正后的正确表述**：15 处中 14 处的比较对象受格式约束为纯 ASCII 且取值域内新旧排序等价；`TradeForm.tsx:351` 的 `platform` 为用户自由文本，**排序在一般取值域上不等价**，其影响范围限于「记账页平台名建议下拉列表的显示顺序」，**不参与任何金额、持仓或派生计算**。

**改动本身保留。** 理由：该处本就只是建议列表排序，确定性排序优于语言相关排序；`07D` 已在三处真正参与派生计算的路径（`cashReplay`／`ledgerDate`／`activityService`）上做了穷举等价性证明，那三处才是风险所在。

### 更正二：R-4 的 `.lftl` 字节相等断言不是承重证据

原文（R-4）把 T-A8「读取 golden `.lftl` 为 `Uint8Array`，切换语言后再次读取，断言前后字节完全相等」作为「语言偏好没有进入账本文件」的主证据。

**该断言在任何实现下都不会变红**——测试 harness 内根本不存在写账本文件的路径，字节不可能变化。它是一条恒真断言，不承担证明责任。

**更正后的正确表述**：B-06（语言偏好不落账本）的承重证据是同一用例中的 `localStorage` **只有一个 key** 这条结构性断言。`07D` 的通电检查第 3 条实测确认：把语言额外写入第二个存储键，该断言立刻变红（1 failed）。**结论不变，且结构性证据比原先援引的字节断言更强。**

### 更正三：R-15 的「不泄漏证明」不能证明复位机制生效

原文（R-15）称 `i18n.test.tsx` 前后两条用例的顺序「证明前一用例的语言没有泄漏到后一用例」。

**该用例无法区分复位机制的两半。** `07D` 实测：单独禁用 `afterEach` 的移除、或单独禁用 `beforeEach` 的钉死，测试**均不变红**（2 passed）；两半同时禁用才变红（1 failed）。

**更正后的正确表述**：钉死与复位两半**互为冗余**，任一半单独失效仍能保住测试隔离——**这是机制的优点，说明保护比合同要求更厚**；但正因如此，该用例只能证明「隔离在当前实现下成立」，**不能证明复位机制本身生效**。机制保留不动。

### 更正四：U-3「首页文案全部经 `t()`」有一处缺口

`07B` 的 U-3 要求「首页文案全部经 `t()`」。实际 `HomeWorkspace.tsx` 有两处标点**硬编码在 `t()` 之外**：
...
全角冒号 `：` 与顿号 `、` 是中文标点。切到英文或匈牙利语时会渲染成 `Update assets without prices：BTC、ETH`，**中日韩标点混进西文排版**。

**更正后的正确表述**：U-3 达成度为「首页全部**词句**经 `t()`，但两处**连接标点**未经 `t()`」。属外观缺口，不影响数据正确性，不构成禁令违反。

**处置：不在本批修**，已按 `08A` A-2「发现真问题记下来，另开批次」的先例登记给 09 批（见 `09A` 第八节）。理由是 09 批本就是文案搬迁批次，标点的语言归属正是它的职责范围；在本批临时补一处会绕开 09 将要冻结的命名与分批规则。

### 一并登记的潜在隐患（非本批缺陷）

`backupImportPreflight.ts:948` 的 `code` 字段类型为宽泛的 `string` 而非字面量联合类型。今日 59 个真实错误码在新旧两种比较器下排序结果相同，**当前无影响**；但下划线与字母的相对次序在两种比较器中相反，**将来新增前缀重叠的错误码可能出现顺序差异**。已登记至 `09A` 第八节。

### 两处报告完整性瑕疵（已由 `07D` 代为核实，此处补记）

- R-7 的 `expect(` 计数表未列 `DashboardShell.golden.test.tsx`——它正是修订 B 的授权改动对象，理应列出。`07D` 已代为核实其计数 **33→33 未变**。
- R-6 的译文表只列了 26 条 `home.*`，未列 6 条 `settings.language.*` 与 1 条 `shared.i18n.fallbackExample`。完整口径为：**中文表 33 键；英、匈各 32 键，仅缺回退示例键，属刻意设计**。设置页文案已在 R-13 另有披露，非遗漏声明。

### 本节不涉及的事项

`07D` 第八节的性能对照（R-10）**验收者未重跑**，本节不为其背书，也不将其复述为已验证结论。`07D` 第十节列出的其余未核验项（开工基线测试条数、匈牙利语译文语言质量、T-A1～T-A7 的逐条通电过程）状态不变。
